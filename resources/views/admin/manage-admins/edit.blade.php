...
<div class="container">
    <div class="admin-page-header">
        <div class="page-title">
            ...
            <p>Update admin details and modify module permissions</p>
        </div>
        <div>
        ...
        <div class="card">
            <form action="{{ route('admin.manage-admins.update', $admin->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    >
                    <span class="text-hint">Minimum 8 characters required if changing</span>
                    @error('password')
                        <span class="error-msg">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group" id="permissions-section">
                    ...
                    <div class="permissions-grid">
                        ...
                        <label class="permission-card {{ old('is_super_admin', $admin->is_super_admin) ? 'disabled' : '' }}" data-permission-card>
                            <input 
                                type="checkbox" 
                                name="permissions[]" 
                                value="{{ $key }}"
                                ...
                                {{ (is_array(old('permissions', $admin->permissions)) && isset(old('permissions', $admin->permissions)[$key]) && old('permissions', $admin->permissions)[$key]) ? 'checked' : '' }}
                                {{ old('is_super_admin', $admin->is_super_admin) ? 'disabled' : '' }}
                            >
        ...
        <div class="card" style="height: fit-content;">
            ...
            <ul class="info-list">
                ...
                <li>
                    ...
                    <span>Modifying permissions updates access immediately.</span>
                </li>
            </ul>
        </div>
    </div>
    ...
    superAdminCheckbox.addEventListener('change', function() {
        ...
        permissionCheckboxes.forEach(checkbox => {
            if (isSuperAdmin) {
                ...
                // If it was checked before disable, we might want to restore state? 
                // For simplicity, we just enable them and let user re-check if needed or rely on form submission handling
            }
        });
    });
</script>
